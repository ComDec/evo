...
def main():
    ...
    ap.add_argument(
        "--prompt", type=str, default=default_prompt, help="Prompt for generation"
    )
    ap.add_argument(
        "--model-name",
        type=str,
    ...
    ap.add_argument(
        ...
        type=int,
        default=1024,
        help="Max new tokens during sampling",
    )
    ap.add_argument(
        "--repetition-penalty",
        type=float,
        default=1.0,
        help="Repetition penalty during sampling",
    )
    ap.add_argument(
        "--penalty-alpha", type=float, default=0.0, help="Penalty alpha during sampling"
    )
    # output args:
    ap.add_argument(
        "--sequence-fasta",
    ...
    ap.add_argument(
        ...
        help="Proteins fasta file",
    )
    ap.add_argument(
        "--structure-pdb", type=str, default="structure.pdb", help="Structure PDB file"
    )
    # misc args:
    ap.add_argument(
    ...
    prompt_ids = (
        torch.tensor(tokenizer.tokenize(args.prompt)).to(torch.long).to(args.device)
    )
    ...
    dna_seq_record = SeqRecord(
        Seq(dna_seq), id="evo-dna", description="DNA sequence generated by Evo."
    )
    ...
    for i, protein_record in enumerate(SeqIO.parse(args.proteins_fasta, "fasta")):
        ...
        protein_struct = bsio.load_structure(
            args.structure_pdb, extra_fields=["b_factor"]
        )
